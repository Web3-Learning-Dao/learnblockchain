...
const value = await params.GetUint256Param("1"); //0
console.log(value);

let getData = iface.encodeFunctionData("GetUint256Param", [ "1" ]);

let tx1 = await alice.sendTransaction({to: transparentUpgradeableProxyContract.address, data: getData});
const res1 = await tx.wait();
console.log("TX1: ", tx1); //2
// expect(value.to.equal(1));


await proxyAdminContract.upgrade(transparentUpgradeableProxyContract.address,paramsNew.address );

const value1 = await paramsNew.GetUint256Param("1");//1
console.log(value1);

let tx2 = await alice.sendTransaction({to: transparentUpgradeableProxyContract.address, data: getData});
const res2 = await tx.wait();
console.log("TX2: ", tx2);//3
